...
const { MLPClassifier, MLPRegressor } = require('../src/index.js')
...
await test('binary classification', async () => {
  ...
  assert(model.score(X, y) > 0.9, `accuracy should be > 0.9, got ${model.score(X, y)}`)
  model.dispose()
})
...
await test('defaultSearchSpace', async () => {
  ...
  assert(space.batch_size, 'missing batch_size')
})

await test('shuffled epoch training', async () => {
  const { X, y } = makeBinaryData(42, 64)
  const model = await MLPClassifier.create({
    hidden_sizes: [8], epochs: 50, lr: 0.01,
    optimizer: 'sgd', seed: 42
  })
...
await test('early stopping terminates early', async () => {
  const { X, y } = makeBinaryData(42, 100)
  const model = await MLPClassifier.create({
    hidden_sizes: [8], epochs: 1000, lr: 0.01,
    optimizer: 'sgd', seed: 42,
    validation_fraction: 0.2, patience: 5
  })
  const t0 = Date.now()
  model.fit(X, y)
  const elapsed = Date.now() - t0
  assert(model.isFitted, 'should be fitted')
  assert(elapsed < 60000, `should complete with early stopping, took ${elapsed}ms`)
  model.dispose()
})

await test('save/load preserves batchSize', async () => {
  const { X, y } = makeBinaryData(42, 32)
  const model = await MLPClassifier.create({
    hidden_sizes: [4], epochs: 10, lr: 0.01,
    optimizer: 'sgd', seed: 42
  })
  ...
  const bundleBytes = model.save()
  const { manifest } = decodeBundle(bundleBytes)
  assert(manifest.metadata.batchSize === 1, `batchSize should be 1, got ${manifest.metadata.batchSize}`)

  const loaded = await MLPClassifier.load(bundleBytes)
  const preds1 = model.predict(X)
  const preds2 = loaded.predict(X)
  for (let i = 0; i < preds1.length; i++) {
    assert(preds1[i] === preds2[i], `prediction mismatch at ${i}`)
  }
...
await test('capabilities includes earlyStopping', async () => {
  const model = await MLPClassifier.create()
  assert(model.capabilities.earlyStopping === true, 'earlyStopping should be true')
  model.dispose()
})
...
await test('regression fit and score', async () => {
  ...
  assert(r2 > 0.9, `R2 should be > 0.9, got ${r2}`)
  model.dispose()
})
...
await test('shuffled epoch training', async () => {
  const { X, y } = makeRegressionData(42, 64)
  const model = await MLPRegressor.create({
    hidden_sizes: [16], epochs: 100, lr: 0.01,
    optimizer: 'sgd', seed: 42
  })
...
await test('early stopping terminates early', async () => {
  const { X, y } = makeRegressionData(42, 100)
  const model = await MLPRegressor.create({
    hidden_sizes: [16], epochs: 1000, lr: 0.01,
    optimizer: 'sgd', seed: 42,
    validation_fraction: 0.2, patience: 5
  })
  const t0 = Date.now()
...
await test('save/load preserves batchSize', async () => {
  const { X, y } = makeRegressionData(42, 32)
  const model = await MLPRegressor.create({
    hidden_sizes: [4], epochs: 10, lr: 0.01,
    optimizer: 'sgd', seed: 42
  })
  ...
  const preds2 = loaded.predict(X)
  for (let i = 0; i < preds1.length; i++) {
    assertClose(preds1[i], preds2[i], 1e-5, `prediction mismatch at ${i}`)
  }
