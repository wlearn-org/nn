...
// Seeded LCG PRNG for deterministic shuffling
function lcg(seed) {
  let s = seed
  return function () {
...
class MLPClassifier {
  ...
  fit(X, y) {
    ...
    // Mini-batch (batch_size > 1) will be enabled once polygrad supports it.
    const batchSize = this.#params.batch_size || 1

    const layers = [nFeatures, ...hidden, this.#nrClass]
    const spec = {
      layers, activation, bias: true,
      loss: 'cross_entropy', batch_size: batchSize, seed
    }
...
class MLPRegressor {
  ...
  fit(X, y) {
    ...
    const spec = {
      layers, activation, bias: true,
      loss: 'mse', batch_size: batchSize, seed
    }
  ...
  predict(X) {
    ...
    // Determine nOutputs from first forward pass
    batchX.fill(0)
