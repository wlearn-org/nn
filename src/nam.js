/**
 * NAMClassifier and NAMRegressor for wlearn using polygrad Instance backend.
 *
 * NAM: Neural Additive Model.
 *   g(E[y]) = beta + f1(x1) + f2(x2) + ... + fK(xK)
...
class NAMClassifier {
  ...
  fit(X, y) {
    ...
    const activation = this.#params.activation || 'exu'
    ...
    this.#instance = Instance.nam(spec)
  ...
  #ensureFitted() {
    if (this.#disposed) throw new DisposedError('NAMClassifier has been disposed.')
    if (!this.#fitted) throw new NotFittedError('NAMClassifier is not fitted.')
  }
...
class NAMRegressor {
  ...
  #ensureFitted() {
    if (this.#disposed) throw new DisposedError('NAMRegressor has been disposed.')
    if (!this.#fitted) throw new NotFittedError('NAMRegressor is not fitted.')
  }
